...
import com.mingjiang.android.app.util.NetCheckUtils;
...
//    Button mButton;
    ...
    private void initForIsNetWork() {
        if (!NetCheckUtils.isNetworkAvaiable(InspectionMainActivity.this) && !NetCheckUtils.isWifiActive(InspectionMainActivity.this)) {
...
//                Toast.makeText(InspectionMainActivity.this, "正在登陆", Toast.LENGTH_SHORT).show();
//                if ("123".equals(mEdittext1.getText().toString()) && "123".equals(mEdittext2.getText().toString())) {
//                    Intent intent = new Intent(InspectionMainActivity.this, CheckActivity.class);
//                    intent.putExtra("username", mEdittext1.getText().toString());
//                    startActivity(intent);
//                } else {
//                    Toast.makeText(InspectionMainActivity.this, "登录失败，请重新扫码或输入工号", Toast.LENGTH_SHORT).show();
//                }
//            }
//        });
    }
...
//        builder.setIcon(R.drawable.meiling);
        ...
        builder.setPositiveButton("是",
                new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        //开启服务，读取用户
                        ComServiceUtils.startService(InspectionMainActivity.this, ComEvent.INSPECTION_USERSCAN);
                    }
                });
    ...
    protected void onResume() {
        super.onResume();
        String postCode = SharedPrefsUtil.getString(this, AppConfig.POST_CODE);
    ...
    public void startUserScanActivity(String userCode) {
        AppContext.getApp().getNetService(this).isUser(userCode, "{\"verify\":0}").subscribe(new Action1<SessionObjForPost>() {
            @Override
            public void call(SessionObjForPost retValue) {
    ...
    public void onEventMainThread(ComEvent event) {
        if (event.getActionType() == ComEvent.CHECK_USER) {
            SessionObjForPost sessionObj = (SessionObjForPost) event.getObjectMsg();
            ...
            Toast.makeText(InspectionMainActivity.this, "用户校验出错", Toast.LENGTH_SHORT).show();
            ...
            SharedPrefsUtil.setValue(this, AppConfig.POST_CODE, postCode);//将PostCode保存本地
            startUserScanActivity(postCode);
        }
    }
    @Override
