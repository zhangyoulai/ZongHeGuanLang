...
import com.mingjiang.android.app.adapter.MyAdapter;
import com.mingjiang.android.app.bean.FilePath;
import com.mingjiang.android.app.bean.FridgeParamsBean;
import com.mingjiang.android.app.bean.FridgePostBean;
import com.mingjiang.android.app.bean.FridgeResponse;
import com.mingjiang.android.app.bean.YieldData;
import com.mingjiang.android.app.view.CallbackBundle;
import com.mingjiang.android.app.view.OpenFileDialog;
...
public class InstructionPdfActivity extends Activity implements OnPageChangeListener, OnLoadCompleteListener {
    protected final static String TAG = InstructionPdfActivity.class.getSimpleName();
    private String postCode = "";
    private String sessionId = "";
    private Dialog loadingPdf;
    //试用工厂
...
//        });
    ...
    class QueryClick implements View.OnClickListener {
        ...
        public void onClick(View v) {
            getPDFInfo(editText.getText().toString(),postCode);
//            getPDFInfo("",postCode);
        }
    }
...
//            playPdf(file.getPath());
    ...
    private void dealDataToView(FridgeResponse fridgeResponse) {
        Log.i(TAG,fridgeResponse.toString());
        //适用工厂
        String applyFactory = "";
        ...
        try {
            ...
            Toast.makeText(InstructionPdfActivity.this, e.toString(), Toast.LENGTH_SHORT).show();
        }
     ...
     *
     */
    private void getPDFInfo(final String fridgeCode, final String postCode) {
        Map<String, String> map = new HashMap<>();
        map.put("product_type",fridgeCode);
        map.put("post",postCode);
        AppContext.getApp().getNetService(this).getPDFInfo(map).subscribe(new Action1<FridgeResponse>() {
            @Override
    ...
    private void sendFridgePostBean(final String fridgeCode, final String postCode, final String sessionId) {
        ...
        Log.i(TAG,sessionId);
        AppContext.getApp().getNetService(this).sendFridgePostBean(fridgeParamsBean, sessionId).subscribe(new Action1<FridgeResponse>() {
            @Override
            public void call(FridgeResponse fridgeResponse) {
                //加载数据成功
                String sendMsg = "岗位编号为：" + postCode + "-冰箱编号为：" + fridgeCode + "-上传成功！";
    ...
    private void getYieldData(String produce_post, String serial_number) {
        ...
        AppContext.getApp().getNetService(this).getYieldData(map, sessionId).subscribe(new Action1<List<YieldData>>() {
            @Override
...
//            images.put("pdf", R.drawable.skin_blue); // mp4文件图标
//            images.put(OpenFileDialog.sEmpty, R.drawable.skin_unusual);
//            loadingPdf = OpenFileDialog.createDialog(id, this, "打开文件",
//                    new CallbackBundle() {
//                        @Override
//                        public void callback(Bundle bundle) {
//                            String path = bundle.getString("path");
//                            Log.e("cb--->", "pdf路径" + path);
//                            playPdf("/mnt" + path);//加载pdf
//                        }
//                    }, ".pdf;", images);
//            return loadingPdf;
//        }
...
//    public void playPdf(String url) {
