...
import com.mingjiang.android.app.asynctask.RequireAsyncTask;
import com.mingjiang.android.app.asynctask.SubmitAsyncTask;
import com.mingjiang.android.app.bean.ChosePostInfo;
...
public class CheckActivity extends Activity {
    ...
    protected final static String TAG = CheckActivity.class.getSimpleName();

    List<TextView> mList;//用来存放人员检测结果
    List<TextView> mDeviceList;//用来存放设备检测结果
    List<TextView> mIndexList;//用来存放指示当前岗位的textview
    TextView textView11,textView12,textView13,textView45,myPostTextView,mDeviceFiveTest,mTextviewLocation,mTextViewrReason;
    String mProduct_id;
...
//            "shock_result": false, "wifi_result": false, "screen_result": false, "infrared_result": false, "property3_result": false, "dynamicd_result": false}
    ...
    protected void onResume() {
        super.onResume();
        mPost= SharedPrefsUtil.getInt(CheckActivity.this, AppConfig.CHECK_POST_CODE);
        mClasses=SharedPrefsUtil.getInt(CheckActivity.this, AppConfig.CHECK_CLASSES_CODE);
        if(mPost==0){//本地文件不存在
            mPost=0;
    ...
    private void ininViewForSpinner() {
        ...
        spinner14.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view,
                ...
                AppContext.getApp().getNetService(CheckActivity.this).isInspectionMaster(mUserName, AppConfig.sessionId).subscribe(new Action1<HaveAuthority>() {
                    @Override
        ...
        spinner43.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view,
                ...
                mClasses = position;
                SharedPrefsUtil.setValue(CheckActivity.this, AppConfig.CHECK_CLASSES_CODE, mClasses);
                //  initViewForMyPost();
    ...
    public void onEventMainThread(ComEvent event) {
        ...
        if (event.getActionType() == ComEvent.INSPECTION_CHECKSCAN) {
            ...
            SharedPrefsUtil.setValue(this, AppConfig.CHECK_POST_CODE, mPost);//将PostCode保存本地
            ...
            if (info.getResult()==1){
                ...
                SharedPrefsUtil.setValue(CheckActivity.this, AppConfig.CHECK_POST_CODE, mPost);
                initViewForMyPost();
    ...
    public void showProductInfo(String product_id) {
        ...
        String url = Config.getBaseUrl(CheckActivity.this)+AppConfig.PRODUCT_URL + product_id+"/"+(mPost+1);
...
//                             "bad_reason": "箱体正面（24面、梁）磕碰、划痕、折、外翻、掉漆",
        ...
        }).execute(url, AppConfig.sessionId);
            ...
            public void onReasonListener(boolean result) {
                if (result==true){
                    ...
                    if ("1".equals(mResult_code)){
                        ...
                        textView45.setText("当前状态：检测合格");
                    }else{
                        myPostTextView.setBackgroundResource(R.color.red);
                        textView45.setText("当前状态：检测不合格");
                    }
                }else{
                    Toast.makeText(CheckActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
                    textView45.setText("当前状态：提交失败");
                }
                initForCheckData();
            }
        }).execute(Config.getBaseUrl(CheckActivity.this)+AppConfig.RESULT_URL,submitData,AppConfig.sessionId);
    }
    private void initViewForFind() {
        ...
        textView45=(TextView)findViewById(R.id.activity_check_text45);
