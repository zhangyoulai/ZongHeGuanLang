...
import com.mingjiang.android.app.adapter.OneKeyAddAdapter;
...
public class AddLibraryFragment extends Fragment {
    ...
    private void queryMaterial(String code){
        AppContext.getApp().getNetService(context).queryMaterial(code).subscribe(new Action1<List<MaterialOneKeyAddResult>>() {
            @Override
    ...
    public void dealSubmit(final MaterialOneKeyAddResult materialOneKeyAddResult){
        AppContext.getApp().getNetService(context).submitMaterial(materialOneKeyAddResult.code,materialOneKeyAddResult.number).subscribe(new Action1<String>() {
            @Override
